...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  realpath(root_dir.c_str(), root_buf);
  realpath(test_file.c_str(), test_buf);

  if (root_buf == NULL || test_buf == NULL) {
    return false;
  }

  size_t root_len = strlen(root_buf);
  size_t test_len = strlen(test_buf);

  if (root_len >= test_len) {
    return false;
  }

  if (strncmp(root_buf, test_buf, root_len) != 0) {
    return false;
  }

  vector<string> root;
  vector<string> test;
  boost::split(root, root_dir,
               boost::is_any_of("/"),
               boost::token_compress_on);
  boost::split(test, test_file,
               boost::is_any_of("/"),
               boost::token_compress_on);

  for (u_int64_t i = 0; i < root.size(); i++) {
    if (strcmp(root[i].c_str(), test[i].c_str()) != 0) {
      return false;
    }
  }
