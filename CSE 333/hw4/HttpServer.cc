...
static void HttpServer_ThrFn(ThreadPool::Task* t) {
  ...
  while (!done) {
    ...
    string c("connection");
    if (strcmp(req.GetHeaderValue(c).c_str(), "close") == 0) {
      close(hst->client_fd);
      done = true;
    }
  }
...
static HttpResponse ProcessFileRequest(const string& uri,
  ...
  file_name.replace(0, strlen("/static/"), "");
...
static HttpResponse ProcessQueryRequest(const string& uri,
  ...
  if (uri.find("query?terms=") != string::npos) {
    ...
    boost::split(queries, query,
                 boost::is_any_of(" "),
                 boost::token_compress_on);

    hw3::QueryProcessor qr(indices, false);

    vector<hw3::QueryProcessor::QueryResult> pq;
    pq = qr.ProcessQuery(queries);

    size_t pq_num = pq.size();
    if (pq_num == 0) {
      ...
      for (auto &doc : pq) {
        ...
        if (doc.document_name.substr(0, 7) != "http://") {
          ret.AppendToBody("/static/");
        }
        ret.AppendToBody(doc.document_name);
        ret.AppendToBody("\">");
        ret.AppendToBody(EscapeHtml(doc.document_name));
        ret.AppendToBody("</a> [");
        ss << doc.rank;
        ret.AppendToBody(ss.str());
        ss.str(string());
