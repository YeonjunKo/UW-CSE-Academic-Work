...
// This function reads user input of queries
// and process them until eof input.
// Parameter:
//  - qp: Query Processor to process list of queries
static void ProcessQueries(hw3::QueryProcessor const &qp);

// This function makes a vector of queries from user input,
// divided by whitespace.
// Parameter:
//  - query_input: user input to split into queries.
// Returns:
//  - This function returns a vector of string that
//    split user input to individual queries based on whitespace.
static vector<string> GetNextLine(string query_input);
...
int main(int argc, char** argv) {
  ...
  hw3::QueryProcessor qp(index_list);
...
static void ProcessQueries(hw3::QueryProcessor const &qp) {
  while (1) {
    ...
    vector<string> query_list = GetNextLine(query_input);

    if (query_list.empty()) {
      continue;
    }

    vector<hw3::QueryProcessor::QueryResult> res = qp.ProcessQuery(query_list);
...
static vector<string> GetNextLine(string query_input) {
  int line_size = query_input.size();
  for (int i = 0; i < line_size; i++) {
    query_input[i] = tolower(query_input[i]);
  }

  vector<string> ret_list;
  istringstream iss(query_input);
  string token;

  while (getline(iss, token, ' ')) {
    ret_list.push_back(token);
  }
