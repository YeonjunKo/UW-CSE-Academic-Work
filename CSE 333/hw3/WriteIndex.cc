...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  while (left_to_read > 0) {
    int read_byte = kBufSize;
    if (left_to_read < kBufSize) {
      read_byte = left_to_read;
    }

    Verify333(fread(&buf, read_byte, 1, f) == 1);
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    int rec_size = WriteHTBucketRecord(f, record_pos,
                   LinkedList_NumElements(ht->buckets[i]), bucket_pos);
    if (rec_size < 0) {
      return kFailedWrite;
    }

    int buc_size = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
    if (buc_size < 0) {
      return kFailedWrite;
    }
...
static int WriteHTBucketRecord(FILE* f, IndexFileOffset_t offset,
  ...
  BucketRecord buc_rec(num_elts, bucket_offset);
  buc_rec.ToDiskFormat();
  ...
  if (fwrite(&buc_rec, sizeof(BucketRecord), 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    ElementPositionRecord elem_rec(element_pos);
    elem_rec.ToDiskFormat();
    if (fseek(f, record_pos, SEEK_SET) != 0) {
      return kFailedWrite;
    }
    if (fwrite(&elem_rec, sizeof(ElementPositionRecord), 1, f) != 1) {
      return kFailedWrite;
    }

    // STEP 8.
    // Write the element itself, using fn.
    LLPayload_t elem = NULL;
    LLIterator_Get(it, &elem);
    int element_size = fn(f, element_pos,
                          reinterpret_cast<HTKeyValue_t *>(elem));
    if (element_size < 0) {
      return kFailedWrite;
    }
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    int64_t truncate = reinterpret_cast<uintptr_t>(pos);
    DocIDElementPosition position(static_cast<DocPositionOffset_t>(truncate));
